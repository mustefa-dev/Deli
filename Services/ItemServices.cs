...
public class ItemServices : IItemServices
{
    ...
    public async Task<(Item? item, string? error)> Create(ItemForm itemForm, string language)
    {
        ...
        if (category == null)
            return (null,
                ErrorResponseException.GenerateLocalizedResponse("Category not found", "لم يتم العثور على الفئة",
                    language));
        var inventory = await _repositoryWrapper.Inventory.Get(i => i.Id == itemForm.InventoryId);
        if (inventory == null)
            return (null,
                ErrorResponseException.GenerateLocalizedResponse("Inventory not found", "لم يتم العثور على المخزن",
                    language));
        item.RefNumber = GenerateRefNumber(category.Name);
        var result = await _repositoryWrapper.Item.Add(item);
        if (result == null)
            return (null,
                ErrorResponseException.GenerateLocalizedResponse("Error in Creating a Item", "خطأ في انشاء العنصر",
                    language));
    ...
    public async Task<(ItemDto? item, string? error)> GetById(Guid userId, Guid id, string language)
    {
        ...
        if (item == null)
            return (null,
                ErrorResponseException.GenerateLocalizedResponse("Item not found", "لم يتم العثور على العنصر",
                    language));
        ...
        var wishlistedItem =
            await _repositoryWrapper.Wishlist.Get(l => l.UserId == userId && l.ItemsIds.Contains(itemDto.Id));
        ...
        itemDto.IsAddedToCart =
            await _repositoryWrapper.Cart.Get(c =>
                c.UserId == userId && c.ItemOrders.Any(i => i.ItemId == itemDto.Id)) != null;
    ...
    public async Task<(List<ItemDto> items, int? totalCount, string? error)> GetAll(Guid userId, ItemFilter filter,
        string language)
    {
        filter.Name = filter.Name?.ToLower();
        var (item, totalCount) = await _repositoryWrapper.Item.GetAll<ItemDto>(
            x => (string.IsNullOrEmpty(filter.Name) || (x.Name.ToLower().Contains(filter.Name) ||
                                                        x.ArName.ToLower().Contains(filter.Name)
                                                        || x.ItemTags.Any(t =>
                                                            t.Tag.Name.ToLower().Contains(filter.Name)) ||
                                                        x.RefNumber.ToLower().Contains(filter.Name))) &&
        ...
        foreach (var itemDto in item)
        {
            ...
            itemDto.GovernorateName =
                ErrorResponseException.GenerateLocalizedResponse(governorate.Name, governorate.ArName, language);
            itemDto.Name =
                ErrorResponseException.GenerateLocalizedResponse(originalItem.Name, originalItem.ArName, language);
            itemDto.MainDetails =
                ErrorResponseException.GenerateLocalizedResponse(originalItem.MainDetails, originalItem.ArMainDetails,
                    language);
            itemDto.Description =
                ErrorResponseException.GenerateLocalizedResponse(originalItem.Description, originalItem.ArDescription,
                    language);
            itemDto.IsAddedToCart =
                await _repositoryWrapper.Cart.Get(c =>
                    c.UserId == userId && c.ItemOrders.Any(i => i.ItemId == itemDto.Id)) != null;
            if (itemDto.IsAddedToCart == true)
            {
                ...
                var itemOrder =
                    await _repositoryWrapper.ItemOrder.Get(i => i.CartId == cart.Id && i.ItemId == itemDto.Id);
                itemDto.QuantityAddedToCart = itemOrder.Quantity;
            }
    ...
    public async Task<(ItemDto? item, string? error)> Update(Guid id, ItemUpdate itemUpdate, string language)
    {
        ...
        if (result == null)
            return (null,
                error: ErrorResponseException.GenerateLocalizedResponse("Error in updating item", "خطأ في تحديث العنصر",
                    language));
    ...
    public async Task<(Item? item, string? error)> Delete(Guid id, string language)
    {
        ...
        if (result == null)
            return (null,
                ErrorResponseException.GenerateLocalizedResponse("Error in deleting item", "خطأ في حذف العنصر",
                    language));
        return (result, null);
    }
        ...
        string language)
    {
        ...
        {
            return (null, 0,
                ErrorResponseException.GenerateLocalizedResponse("Wishlist is empty", "القائمة المفضلة فارغة",
                    language));
        }
        ...
        string language)
    {
        ...
        {
            return (null, 0,
                ErrorResponseException.GenerateLocalizedResponse("Liked list is empty", "قائمة الإعجاب فارغة",
                    language));
        }
    ...
    public async Task<(Sale? sale, string? error)> AddSaleToItem(Guid itemId, double salePrice, DateTime startDate,
        DateTime endDate, string language)
    {
        var item = await _repositoryWrapper.Item.GetById(itemId);
        if (item == null)
        {
        ...
        {
            return (null,
                ErrorResponseException.GenerateLocalizedResponse("Item already has a sale scheduled during this period",
                    "العنصر لديه تخفيض مجدول خلال هذه الفترة", language));
        }
    ...
    public async Task<(Sale? sale, string? error)> EndSale(Guid itemid, string language)
    {
        ...
        {
            return (null,
                ErrorResponseException.GenerateLocalizedResponse("Sale either doesn't exist or it is not active",
                    "لم يتم العثور على التخفيض او التخفيض غير نشط", language));
        }
    ...
    public async Task<(Sale? sale, string? error)> DeleteScheduledSale(Guid saleId, string language)
    {
        ...
        {
            return (null,
                ErrorResponseException.GenerateLocalizedResponse("Sale not found", "لم يتم العثور على التخفيض",
                    language));
        }
        ...
        {
            return (null,
                ErrorResponseException.GenerateLocalizedResponse("Sale already ended", "انتهى التخفيض بالفعل",
                    language));
        }
        ...
        {
            return (null,
                ErrorResponseException.GenerateLocalizedResponse("Sale already started", "بدأ التخفيض بالفعل",
                    language));
        }
        ...
        {
            return (null,
                ErrorResponseException.GenerateLocalizedResponse("Error in deleting sale", "خطأ في حذف التخفيض",
                    language));
        }
        ...
        string language)
    {
        ...
        {
            return (null,
                ErrorResponseException.GenerateLocalizedResponse("No sold items found", "لم يتم العثور على عناصر مباعة",
                    language));
        }
    ...
    public async Task<(PriceRangeDto priceRangeDto, string? error)> GetPriceRange(string language)
    {
        ...
        {
            return (null,
                ErrorResponseException.GenerateLocalizedResponse("No items found", "لم يتم العثور على عناصر",
                    language));
        }
    ...
    private string GenerateRefNumber(string categoryName)
    {
        
        var lastRef = _context.RefNumbers.FirstOrDefault();
        if (lastRef == null)
        {
            lastRef=new SequentialNumbers();
            _context.RefNumbers.Add(lastRef);
        }
       lastRef.LastRefNumber++;
        _context.SaveChanges();
        return "#"+categoryName.Substring(0, 3).ToUpper() + lastRef.LastRefNumber.ToString();
